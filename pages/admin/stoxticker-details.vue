          ...
          >
            <h5 class="card-title">
              ...
              <button
                v-if="user.id != stoxtickerDetails.board.user_id"
                :class="
                  (boardFeatured ? 'theme-green-btn' : 'theme-btn') + ' card-btn'
                "
                @click="featuredBoard()"
              >
                {{ boardFeatured ? 'Unfeatured' : 'Featured' }}
              </button>
...
export default {
  ...
  methods: {
          ...
          .then((res) => {
            if (res.status == 200) {
              ...
              if (intialTime == 1) {
                ...
                this.boardFeatured = res.board.featured == 1 ? true : false
              }
    ...
    featuredBoard() {
      this.$axios
        .$post('stoxticker/add-feature', { board_id: this.$route.query.board })
        .then((res) => {
          if (res.status == 200) {
            this.boardFeatured = res.board
            this.$toast.success('Stoxticker board updated successfully.')
          }
        })
    },
